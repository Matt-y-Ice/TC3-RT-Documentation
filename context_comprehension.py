...
def extract_injury_details(text):
    ...
    injury_data = []
    pattern = re.compile(
        r"\b(?:(left|right)\s+)?(arm|leg|forearm|thigh|knee)\b", re.IGNORECASE
    )
        ...
        injury_data.append(
            {
                ...
                "extremity": ext,
                "location_phrase": (
                    f"{laterality} {part}" if laterality else part
                ).strip(),
            }
        )
    return injury_data
...
def extract_tourniquet_details(text):
    ...
    for sent in doc.sents:
        if any("tourniquet" in token.lemma_.lower() for token in sent):
            ...
            status = (
                "possible tourniquet placement"
                if any(token.lemma_.lower() in suggestion_keywords for token in sent)
    ...
    for sent_text, status in tourniquet_sentences:
        matched = (
            ...
            if anatomy_mentions
                ...
                "laterality": None,
                "extremity": None,
                "location_phrase": "unspecified",
            }
        )

        if status == "confirmed tourniquet placement":
            CONFIRMED_PLACEMENT = (
                f"Confirmed tourniquet placement at {matched['location_phrase']}"
            )
...
def assemble_prediction_string(injury_data, tourniquet_data):
    ...
    for tq in tourniquet_data:
        if tq["status"] == "confirmed tourniquet placement":
            predictions.append(
                f"Confirmed tourniquet placement at {tq['location_phrase']}"
            )
        else:
            predictions.append(
                f"Suggested/Questioned tourniquet placement at {tq['location_phrase']}"
            )
