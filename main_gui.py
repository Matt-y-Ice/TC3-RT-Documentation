...
from video_pipeline import TourniquetObserver
...
MAX_UTTERANCE_DURATION = (
    10.0  # maximum seconds to accumulate before forcing transcription
)
...
class App:
    ...
    def setup_gui(self):
        ...
        # Button panel –‑ centered in column 1, row 1
        self.button_panel = Frame(self.main_frame, bg="#b0bec5")
        self.button_panel.grid(row=1, column=1)      # ← no “sticky”

        # Put the buttons next to one another and leave them centered
    ...
    # ───────── unified text‑append helper ─────────
    def post_detection(self, line: str):
        """Append one line to text_area2 from any thread."""
        self.add_text_to_text_area2('Model: ' + line)
        if not line.endswith("\n"):
            line += "\n"
        self.root.after(0, lambda txt=line: self._append_detection(txt))

    def _append_detection(self, txt: str):
        self.text_area2.config(state="normal")
        self.text_area2.insert(tk.END, txt)
        self.text_area2.see(tk.END)
        self.text_area2.config(state="disabled")
    ...
    def toggle_transcription(self):
        ...
        if self.toggle_button['text'] == 'Start Demo':
            self.toggle_button.config(text='Stop Demo')
        else:
            self.toggle_button.config(text='Start Demo')
            ...
            while not self.stop_event.is_set():
                try:
                    ...
                    if "Frame didn't arrive" in str(e):
                        ...
                        if current_time - last_warn > 1:
                            logger.warning(
                                "[Capture] Frame timeout, attempting to recover..."
                            )
                            last_warn = current_time
                        try:
                            pipeline.stop()
                            ...
                            logger.error(
                                f"[Capture] Failed to restart pipeline: {restart_error}"
                            )
                    else:
    ...
    def _capture_webcam(self, logger):
        ...
        if not cap.isOpened():
            logger.error(
                "[Capture] Failed to open webcam with index %d and backend %d",
                index,
                backend,
            )
            self._display_no_camera_message()
        ...
        logger.info(
            "[Capture] Webcam started successfully with index %d and backend %d",
            index,
            backend,
        )
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if not ret:
                    ...
                    if current_time - last_warn > 1:
                        logger.warning(
                            "[Capture] Webcam read failed – waiting for device…"
                        )
                        last_warn = current_time
                    time.sleep(0.05)
                    continue
    ...
    def detect_motion(self, current_frame, last_frame):
        ...
        _, motion_mask = cv2.threshold(
            frame_diff, MOTION_THRESHOLD, 255, cv2.THRESH_BINARY
        )
        ...
        if self.active_tracks:
            ...
            for i, track in enumerate(self.active_tracks):
                for j, det in enumerate(detections):
                    cost_matrix[i, j] = self.calculate_iou(track["bbox"], det[:4])
            ...
            self.active_tracks = [
                t for t in self.active_tracks if t["age"] <= MAX_TRACK_GAP
            ]

        # Create new tracks for unassigned detections
        for i, det in enumerate(detections):
            if (
                i not in assigned_detections and len(det) >= 6
            ):  # Ensure det has all required elements
                new_track = {
    ...
    def process_frames(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                def detection_worker():
                    try:
                        ...
                        if self.last_frame is not None:
                            # Preprocess frames
                            current_frame = self.preprocess_frame(
                                batch_for_detection[0]
                            )
                            ...
                            self.current_gray = cv2.cvtColor(
                                current_frame, cv2.COLOR_BGR2GRAY
                            )
                            self.last_gray = cv2.cvtColor(
                                last_frame, cv2.COLOR_BGR2GRAY
                            )
                            feature_matches = self.match_features(
                                self.detect_features(current_frame), self.last_keypoints
                            )

                        # Multi-scale detection
                        all_detections = []
                        for scale, weight in zip(
                            SCALE_FACTORS, SCALE_CONFIDENCE_WEIGHTS
                        ):
                            # Resize frame
                                ...
                                if MIN_BOX_AREA <= box_area <= MAX_BOX_AREA:
                                    ...
                                    if motion_boxes:
                                        for motion_box in motion_boxes:
                                            ...
                                            if iou > 0.1:
                                                conf = min(
                                                    conf * MOTION_CONFIDENCE_BOOST, 1.0
                                                )
                        ...
                        if len(batch_frames) > 0:
                            ...
                            self.last_keypoints = self.detect_features(
                                batch_for_detection[0]
                            )
                    ...
                    try:
                        ...
                        for i, result in enumerate(results):
                            if result.keypoints is not None:
                                ...
                                pose_results[i] = [
                                    (x, y, conf) for x, y, conf in keypoints
                                ]
                            else:
                                pose_results[i] = []
                    except Exception as e:
                ...
                if (
                    detection_results[0]
                    and len(self.detection_history) == TEMPORAL_SMOOTHING
                ):
                    valid_tracks = [
                        t
                ...
                for i in range(len(batch_frames)):
                    frame_anno = self.annotate_frame(
                        batch_frames[i], detection_results[i], pose_results[i]
                    )
                ...
                if current_time - last_log_time > 5.0:
                    logger.info(
                        f"[Pipeline] Stats: {len(self.active_tracks)} active tracks, {frame_count} frames processed"
                    )
    ...
    def annotate_frame(self, frame, detections, keypoints):
        ...
        y_scale = (
            RS_HEIGHT if self.cam_kind == "realsense" else FRAME_HEIGHT
        ) / FRAME_HEIGHT
        ...
        for track in self.active_tracks:
            ...
            if (
                len(track["history"]) >= MIN_TRACK_LENGTH
                and track["confidence"] >= MIN_TRACK_CONFIDENCE
                and track["age"] <= 2
            ):
        ...
        if SAVE_DEBUG_VIDEO and self.recording_buffer:
            try:
                ...
                if video_writer.isOpened():
                    ...
                    logger.error(
                        f"[Debug] Failed to open video writer for {output_file}"
                    )
            except Exception as e:
    ...
    def stop_pipeline(self):
        ...
        applied = (
            len(self.tourniquet_observer.applied_tracks)
            if self.tourniquet_observer
            else 0
        )
        logger.info(
